...
function spotkanie_potok() {
    if $3; then
        cat $2 >>/dev/null 
    else 
        echo 'Dane przesłane w trakcie spotkania' >>$2
        rm $2
    fi
    komunikat "$1 Doszło do spotkania z wykorzystaniem potoku: $2"
}
...
readonly LICZBA_WYMAGANYCH_PARAMETROW=8
